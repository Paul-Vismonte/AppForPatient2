  ...
  useImperativeHandle(focusElementRef, () => ({
    focus: () => {
      if (ratingRef.current) {
        const input = ratingRef.current.querySelector(`input[value="${Number(item.value) || ''}"]`) as HTMLInputElement | null;
        if (input) {
          input.focus();
        }
      }
...
const ratingOnlyOperators: GridFilterOperator[] = [
  {
    ...
    getApplyFilterFn: (filterItem: GridFilterItem) => {
      ...
      return (params: { value: number }): boolean => Number(params.value) >= Number(filterItem.value);
    },
