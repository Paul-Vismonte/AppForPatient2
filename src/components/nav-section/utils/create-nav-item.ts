...
type CreateNavItemReturn = {
  ...
  rootItem: boolean;
  subDeepItem: boolean;
  baseProps: { component: 'div' } | { component: typeof RouterLink; href: string } | { href: string; target: '_blank'; rel: 'noopener' };
...
export function createNavItem({
  ...
  const linkProps = externalLink
    ? { href: path, target: '_blank' as const, rel: 'noopener' as const }
