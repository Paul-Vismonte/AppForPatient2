...
export function OrganizationalChart<T extends { name: string; children: T[] }>({ data, nodeItem, ...other }: OrgChartProps<T>) {
...
function TreeList<T extends { children?: T[] }>({ data, depth, nodeItem }: OrgChartListProps<T>) {
...
function TreeSubList<T extends { children?: T[] }>({ data, depth, nodeItem }: OrgChartSubListProps<T>) {
  return (
    <>
      {data.map((list, index) => (
...
function flattenArray<T extends { children?: T[] }>(list: T[]): T[] {
  let children: T[] = [];
  ...
  return flatten.concat(children.length ? flattenArray(children) : []);
}
