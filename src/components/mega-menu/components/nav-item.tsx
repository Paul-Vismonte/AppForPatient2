...
export const NavItem = forwardRef<HTMLButtonElement | HTMLAnchorElement, NavItemProps>(function NavItem({
  path,
  icon,
  info,
  title,
  ...
  return (
    <ItemRoot
      ...
      ref={path ? (ref as React.Ref<HTMLAnchorElement>) : (ref as React.Ref<HTMLButtonElement>)}
      aria-label={title}
      {...ownerState}
      {...navItem.baseProps}
...
const shouldForwardProp = (prop: string) => !['active', 'open', 'disabled', 'sx', 'ref'].includes(prop);

/**
 * @slot root
 */
const ItemRoot = styled(ButtonBase, { shouldForwardProp })<StyledState & { component?: React.ElementType }>(({ theme }) => ({
  width: '100%',
